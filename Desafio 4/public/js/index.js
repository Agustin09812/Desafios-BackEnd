...
socket.on('updateProducts', (products) => {
    // Limpiar el contenido actual y mostrar la nueva lista de productos
    productosDiv.innerHTML = ''
    products.forEach(product => {
        productosDiv.innerHTML += `<p>${product.title} - ${product.description} - ${product.price} - ${product.code} - ${product.stock} <button onclick="deleteProduct(${product.id})">Eliminar</button></p>`
    })
})
